...
const App = () => {
  const [chat, setChat] = useState([
    {
      role: { system: null },
      content: "I'm an AI specializing in marine conservation and ocean sustainability. Ask me anything about marine ecosystems, pollution, and conservation efforts."
    }
  ]);
  ...
  const askAgent = async (messages) => {
    try {
      const response = await backend.chatConservation(messages);
      setChat((prevChat) => {
  ...
  const fetchRealTimeMarineData = async () => {
    try {
      const data = await backend.getRealTimeMarineData();
      setChat((prevChat) => [...prevChat, { role: { system: null }, content: `Real-time Marine Data: ${data}` }]);
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-blue-50 p-4">
      <div className="flex h-[80vh] w-full max-w-2xl flex-col rounded-lg bg-white shadow-lg">
        <div className="flex justify-between p-4 bg-blue-500 text-white rounded-t-lg">
          <h1 className="text-lg font-bold">AI Marine Conservation Assistant</h1>
          <button onClick={fetchRealTimeMarineData} className="bg-white text-blue-500 px-3 py-1 rounded">Fetch Data</button>
        </div>
        ...
        <form className="flex rounded-b-lg border-t bg-white p-4" onSubmit={handleSubmit}>
          <input
            ...
            placeholder="Ask about marine conservation ..."
            value={inputValue}
